...
const whiteSpace = /^(?!\s+$).*/ ||  /^(?!\s+$)/ ||  /^\s*\S.*$/ || /^\s*\S[^]*$/ || /^\s*\S[\s\S]*$/


//COMMON VALIDATIONS 
const emailValidation =  Yup.string().required().email().matches(whiteSpace, "Cant'\t be white spaces only");
...
export const securityProfileSchema = Yup.object().shape({
    currentPassword: Yup.string().required('No password provided').matches(whiteSpace, "Cant'\t be white spaces only"),
    newPassword: createPassword,
    confirmPassword:Yup.string()
    .oneOf([Yup.ref('newPassword'), null], 'Passwords must match').matches(whiteSpace, "Cant'\t be white spaces only")
  });

  //PROFILE PAGE USER DETAIL SCHEMA 
  export const showProfileSchema = Yup.object().shape({
    fullName: Yup.string().required().matches(whiteSpace, "Cant'\t be white spaces only"),
    phoneNumber: phoneValidation,
    email:emailValidation,
    address:Yup.string().required().matches(whiteSpace, "Cant'\t be white spaces only"),
    pincode:Yup.string().required().matches(whiteSpace, "Cant'\t be white spaces only"),
...
export const loginSchema = Yup.object().shape({
    email: emailValidation,
    password: Yup.string().required('No password provided').matches(whiteSpace, "Cant'\t be white spaces only")
  });


    // SIGN UP SCHEMA 
export const signUpSchema = Yup.object().shape({
  fullName:Yup.string().required().matches(whiteSpace, "Cant'\t be white spaces only").matches(whiteSpace, "Cant'\t be white spaces only"),
  email: emailValidation,
